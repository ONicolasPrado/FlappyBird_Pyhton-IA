...
pygame.font.init()
FONTE_PONTOS = pygame.font.SysFont('Century725 Cn BT', 30)
...
class Passaro:
    ...
    def desenhar(self, tela):
        ...
        # se o passaro tiver caindo eu não voou bater asa
        if self.angulo <= -80:
            self.imagem = self.IMGS[1]
...
class Cano:
    DISTANCIA = random.randrange(170, 220)
    VELOCIDADE = random.randrange(5, 12)
...
def main(genomas, config): # fitness function
    ...
    while rodando:
            ...
            if not ai_jogando:
                if evento.type == pygame.KEYDOWN:
                    if evento.key == pygame.K_SPACE:
                        for passaro in passaros:
                            passaro.pular()
        indice_cano = 0     # descobrir qual cano olhar
        if len(passaros) > 0:
        ...
        for i, passaro in enumerate(passaros):
            passaro.mover()
            # aunmentar um pouquinho a fitness do passaro
